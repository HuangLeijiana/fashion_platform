...
class ImageSearcher:
    ...
    def search_similar_products(self, query_image_path, top_k=20):
        ...
        for image_path, feature_data in feature_dict.items():
            stored_features = (
                feature_data.get("features") if isinstance(feature_data, dict) else None
            )
            ...
            similarity = float(
                cosine_similarity(query_features.reshape(1, -1), stored_norm.reshape(1, -1))[0][0]
            )
    ...
    def _build_results(self, image_results):
        ...
        for item in image_results:
            ...
            result = product.to_dict() if product else self._fallback_result(filename)
