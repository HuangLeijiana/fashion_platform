...
def search():
    ...
    brands = _get_brands()
    cart_count = (
        sum(item["quantity"] for item in session.get("cart", {}).values())
        if session.get("cart")
        else 0
    )
...
def _perform_product_search(query, category="", brand="", sort_by="relevance", page=1, per_page=20):
    ...
    products_query = Product.query.filter(
        or_(
            ...
            Product.description.like(like_term),
            Product.brand.like(like_term),
        )
    )
...
def _perform_wardrobe_search(query, category="", page=1, per_page=50):
    ...
    offset = (page - 1) * per_page
    items = (
        items_query.order_by(ClothingItem.created_at.desc()).offset(offset).limit(per_page).all()
    )
...
def clear_cart():
    ...
    session.modified = True
    return jsonify(
        {"success": True, "message": "购物车已清空", "total_amount": 0, "total_quantity": 0}
    )
