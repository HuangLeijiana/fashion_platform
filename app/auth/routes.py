...
def _login_error(message, status_code=401):
...
def register():
    ...
    if request.method == 'POST':
        ...
        if not username or not email or not password:
            ...
            return _register_error(message)
...
def _register_error(message, status_code=400):
    if request.is_json:
...
def profile():
    ...
    if not profile_obj:
        profile_obj = UserProfile(user_id=current_user.id, username=current_user.username or current_user.email)
