...
def ai_recommend():
    try:
        ...
        candidates = _build_candidate_pool(static_folder, include_wardrobe, current_user.id)
...
def _build_candidate_pool(static_folder: str, include_wardrobe: bool, user_id: str) -> list[dict]:
    """构建推荐候选池（本地图库 + 可选衣柜）。"""
...
def _compute_similarities(
    ...
    for cand in candidates:
        ...
        similarity = float(
            np.dot(latest_vec, cand_vec) / (np.linalg.norm(latest_vec) * np.linalg.norm(cand_vec))
        )
        ...
        sim_results.append(
            {
                ...
                "wardrobe_item_id": (item.id if cand.get("type") == "wardrobe" else None),
            }
        )
