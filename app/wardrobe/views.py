...
from app.extensions import allowed_file, db, delete_uploaded_file, save_uploaded_file
...
def wardrobe():
    items = (
        ClothingItem.query.filter_by(user_id=current_user.id)
...
def search_clothing():
    ...
    if query:
        ...
        items_query = items_query.filter(
            or_(ClothingItem.name.like(like_term), ClothingItem.brand.like(like_term))
        )
    if category:
...
def batch_add_clothing():
    ...
    existing_hashes = {
        value
        for (value,) in db.session.query(ClothingItem.image_hash)
